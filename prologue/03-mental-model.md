...
3. **Vertex AI** — the model backend. The part that does the actual "thinking" when Claude answers. Razorpay bills centrally, so you don't pay per token.
...
5. **Claude Code** — the harness you actually type into. The terminal app that reads/writes files, runs commands, and talks to Vertex AI through Compass.
...
```
   ...
   └──────────────────────────────────────────────────────────────┘
                            ...
                            ▼
   ┌──────────────────────────────────────────────────────────────┐
   │  LAYER 3  ·  VERTEX AI (the model backend)                   │
   │  Google Cloud-hosted Claude. Razorpay bills centrally.       │
   │  Auth via gcloud. You never handle API keys directly.        │
   └──────────────────────────────────────────────────────────────┘
...
You will not memorise these. Claude will type them for you most of the time. But you need to *recognise* them, because when something fails, the error message is going to land in this layer. "`EACCES: permission denied`" is a Layer 2 error. "`Failed to connect to Vertex`" is a Layer 3 error. "`skill not found`" is a Layer 4 error. Knowing which layer is shouting at you is half the battle.
...
When you type a question to Claude, Claude Code does not have a tiny model on your laptop thinking about your question. It packages your prompt, your file context, and your conversation history into a network request and sends it to **Vertex AI** — Google Cloud's hosted version of Claude. Vertex runs the model, returns a response, and Claude Code shows it to you. Every single time.

Why Vertex and not the Anthropic API directly? Two reasons, both practical:

1. **Centralised billing.** Razorpay has a GCP contract. Your prompts are billed against a single org-level project, not your personal credit card. You never handle an API key.
2. **Network path.** Many Razorpay laptops are behind egress filters and a corporate proxy. Vertex is an approved endpoint through our GCP setup, and authentication flows through `gcloud auth application-default login` — which the org already has policies around. Appendix J — Why Vertex, Why Not Anthropic Direct [coming] will go deeper on this decision.

The one reflex that pays off: **if Claude suddenly stops responding or errors out with "auth" in the message, your Vertex token has likely expired.** Re-running `gcloud auth application-default login` fixes the large majority of them. The rest are corporate-proxy / certificate issues — Appendix D has the incantations.
...
**The mental model shortcut:** *Vertex is the brain. Compass is the education.* Same brain, very different outputs depending on which school it went to.
...
*Likely Layer 3.* Vertex auth expired or network egress is being filtered. Run `gcloud auth application-default login`. If that fails, you're in Layer 2 / network territory — post in the program's Slack channel with your traceroute.
...
If you're a backend engineer or an SRE reading the above and thinking "this is a gross oversimplification" — you are correct, and that is on purpose. Vertex AI is itself a stack of half a dozen things. MCPs have a whole protocol spec. Claude Code has a daemon, a JSON-RPC channel, a session store, etc.

The five-layer model is a **teaching tool**, not an architecture diagram. It's optimised for someone who has never opened a terminal to be able to debug *their own* failures. The appendices go deeper:
...
- Appendix J — Vertex vs. Direct [coming] goes into the auth plane.
...
- Claude Code (Layer 5) is a harness, not the AI. The AI is Vertex (Layer 3). The education is Compass (Layer 4).
...
- Appendix J — Vertex vs. Anthropic Direct [coming]
