...
**Codex.** OpenAI's coding agent. At Razorpay it is an optional secondary surface, not the default quota fallback; use it only when current support guidance confirms access for a bounded task. See [Appendix A — Tool Atlas](../A-tool-atlas/README.md).
...
**GREEN / YELLOW / RED.** The readiness language used across the playbook. White Belt applies it through a seven-check manual setup gate; later modules reuse it as a self-check pattern. See [W.8](../../belts/01-white/W08-green-yellow-red.md).
