...
| Retired Opus-model error (for example, `exceeded budget for model=claude-opus-4-6`) or total `Budget=750.0` exhaustion | For `4-6`/`4-7`, switch the Opus default to `claude-opus-4-8`. For a model-family cap, try Sonnet, an OSS model via LiteLLM, or Codex where appropriate. If the total budget is exhausted, another gateway model, OSS route, or personal Claude Max plan will not bypass it; for business blockers, post in [`#ai-help`](https://razorpay.slack.com/archives/C08C35GKJKD) with manager approval visible; see [D.5](../D-known-issues/README.md#d5--retired-opus-default-still-points-at-claude-opus-4-6-status-workaround) and [D.6](../D-known-issues/README.md#d6--hit-a-model-wise-or-litellm-usage-limit-status-workaround). |
...
*Last reviewed: 2026-07-03. Revision cadence: quarterly, plus on-demand. If a row is stale, ping [`#ai-help`](https://razorpay.slack.com/archives/C08C35GKJKD) and this page gets patched.*
