...
| Have Claude access but no LiteLLM key, no enabled models, or `key can only access models=[...]` | [`#ai-help`](https://razorpay.slack.com/archives/C08C35GKJKD) — ask for LiteLLM gateway provisioning or model enablement; see [D.13](../D-known-issues/README.md#d13--litellm-account-or-model-access-is-not-enrolled-status-fixed) |
...
| An enabled Opus route returns `exceeded budget`, or the total LiteLLM budget is exhausted | A named enabled route with `exceeded budget` has hit its model cap; check usage and use a lower-cost enabled route for routine work. Do not infer that Opus 4.6/4.7 retired or switch to 4.8 solely from that message. If the total budget is exhausted, wait for reset or post an approved business blocker in [`#ai-help`](https://razorpay.slack.com/archives/C08C35GKJKD) with manager approval visible; see [D.5](../D-known-issues/README.md#d5--opus-46-or-47-reports-exceeded-budget-status-workaround) and [D.6](../D-known-issues/README.md#d6--hit-a-model-wise-or-litellm-usage-limit-status-workaround). |
