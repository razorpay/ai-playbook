...
**Freeze the semantic contract before comparing values.** Parity answers “do these two implementations agree?” It does not answer “does this number mean what the label promises?” Write the contract with the domain owner before classifying the legacy path:

1. **Name one business question and milestone.** Define the event or state that earns the count. “Activated” is incomplete until it names the exact milestone; two labels backed by the same event, population, and equation are one measure wearing two hats.
2. **Name the population and attribution rule.** State inclusions, exclusions, and the key that connects an outcome to the product, workflow, or feature named in the metric. A metric called “MCP-assisted” needs an MCP-attribution signal; filtering the whole estate by date is not attribution.
3. **Fix the grain and deduplication rule.** Name what one row of the result represents and how repeated events collapse. For a percentage, the numerator must be a subset of the denominator at the same decision grain. Otherwise, label it as a ratio or frequency—not a rate.
4. **Declare the equation, bounds, and time semantics.** Write the numerator, denominator, unit, event time, timezone, and window. A bounded share or conversion rate outside `0–100%`, a subset larger than its population, or a result many times larger than an owner-known slice is a stop signal, not an interesting insight.
5. **Get meaning approved before implementation parity.** The metric owner approves the contract and one independent anchor or known-answer slice. SQL review can prove that code implements a definition; it cannot choose between two plausible business definitions.

Copy this card into the validation thread before running `compare redesign`:

```text
Metric label and business question:
Decision this metric will inform:
Milestone or event that earns the count:
Population and exclusions:
Attribution signal and entity key:
Result grain and deduplication rule:
Numerator / denominator / unit:
Expected bounds or invariant:
Expected evidence for the validation slice:
Event time / timezone / window:
Independent anchor or known-answer slice:
Metric owner and approval link:

Semantic-contract verdict: approved / stop
```

Stop if the milestone is disputed, attribution is implied only by the label, two differently named metrics resolve to the same contract without owner intent, a bounded share or conversion rate mixes grains or can exceed 100%, or no owner can approve the meaning. Do not use legacy/redesign agreement to overrule a failed semantic check; both queries can reproduce the same bad definition perfectly.
...
2. **Declare the requested range and prove both paths cover it.** Write the exact start and end for a bounded query. If the certified query has no date bound, mark it `UNBOUNDED` (all history) and ask the metric owner whether that is the intended contract. When it is, use the owner-approved metric-inception date or the legacy path's reliable history floor as the requested start. Otherwise, stop until the owner defines a bounded contract. For each producing model, record the earliest date from which its data is reliably complete. Measure that floor from the model's data with the analytics/data owner; do not infer it from the first row returned or from an upstream table's history. Re-measure after a rebuild. If either floor falls after the requested start, mark the result `TRUNCATED` and stop. A recent window where both sides happen to overlap cannot prove parity for a longer query.
3. **Freeze one shared complete window.** Record the latest whole business interval complete on each producing path, then use the earlier boundary as the comparison end. For daily metrics, if either path has only a partial “today,” end at the previous day that is complete on both. Run the exact same start and end on both paths; matching date text is not enough when refresh cutoffs differ.
4. **Prove the comparison is non-vacuous.** Record the evidence each path evaluated at the metric's decision grain: result rows, distinct entities, or numerator and denominator counts. If both sides return no evidence, `0 = 0` proves nothing. Treat the comparison as valid only when the approved semantic contract predicts an empty result for that exact window and filters, and an owner-approved fixture or independent anchor confirms it. Otherwise, stop and ask the metric owner to correct or retire the definition.
5. **Scan shared dates for partial loads.** A date can exist on both sides and still be incomplete. With the analytics/data owner, compare daily counts across the full history both paths can represent, starting at the later source's earliest reliable date. Inspect shortfalls in both directions and set an outlier threshold from that domain's normal daily-delta distribution—not from the 1% metric marker below. For each flagged date, compare distinct business entities at the metric's actual grain. If row counts differ but the entity sets used by the metric match, record a grain artefact rather than claiming missing data. Report absent and partial dates separately from the clean-window computation verdict.
6. **Say `compare redesign`.** Analytics Agent runs the registered legacy and redesign queries with the same inputs and presents the values and percentage delta side by side. If it says no redesign pair is registered, stop; do not invent a table swap.
7. **Read the difference, not just the tick.** A delta above 1% is marked ❌, but that is an investigation flag—not proof that redesign lost. A smaller delta can still matter for money, counts, or narrow segments. Check that both sides use the same grain, filters, frozen complete window, and business population. If legacy is caveated or invalid, compare the result with an owner-approved independent anchor such as a governed definition plus a raw-event count.
8. **Trace freshness and coverage to the producer.** A current timestamp on the final table may prove only that a downstream copy ran. Follow the metric receipt back through the serving table to the table or job that actually produces the data. Confirm that its available partitions cover the full consumer and comparison window—not only the latest day. Then compare at least one value distribution or decision-critical segment. Matching row counts can still hide wrong values after a join.
9. **Record an approve-or-stop decision.** Approve only the metric and window you checked. Attach the comparison receipt, baseline classification, independent anchor when needed, and any accepted owner-confirmed caveat. If redesign intentionally corrects legacy semantics, record the old defect and the owner's approval instead of demanding parity. One green metric does not approve an entire domain.
...
Requested range: bounded <start / end> / UNBOUNDED (all history)
Unbounded intent confirmed by metric owner? yes / no / n/a
Legacy reliably complete from:
Redesign reliably complete from:
Coverage measurement source and date:
Requested-range verdict: COVERED / TRUNCATED / UNKNOWN
Comparable-history floor and source:
Daily completeness grain and entity key:
Normal daily-delta range and approved outlier threshold:
Absent or partial dates flagged:
Entity-key result for each flagged date:
Data-completeness verdict: clean / known defect / stop
...
Legacy evidence count (rows / entities / components):
Redesign evidence count (rows / entities / components):
Empty result expected and owner-approved? yes / no / n/a
Non-vacuous verdict: evidence present / approved empty / stop
...
**Stop conditions.** Stop if the plugin was not refreshed, no registered pair exists, either query fails, the metric intent or domain owner is unknown, the legacy baseline is unclassified, an invalid legacy baseline has no independent anchor, the requested range is undeclared, an unbounded query's intent is unconfirmed, either reliable-complete-from floor is unknown or stale, either path is `TRUNCATED`, the grain or filters differ, either producer's complete-through boundary is unknown, the queries do not share one frozen complete window, both paths return no evidence without an owner-approved expected-empty case and independent anchor, the full comparable history was not checked for absent and partial dates, the daily threshold was copied from the metric-level marker instead of calibrated, a flagged row-count mismatch was not checked at the metric's entity grain, the producing source or refresh owner is unknown, only the downstream copy has a current timestamp, producer coverage is shorter than the consumer window, matching row counts have no value-distribution or segment check, a freshness explanation is unverified, or the result conflicts with an owner-approved source of truth. Keep legacy as primary and route the evidence to the metric owner.

**Why this path exists.** The registered `shadow → compare → promote` contract shipped in [`self-serve-analytics` #1926](https://github.com/razorpay/self-serve-analytics/pull/1926). The first Reporting rollout then asked a PM to run the comparison and sign off only when values match in [`#analytics-self-serve`](https://razorpay.slack.com/archives/C0A98PQTJH4/p1785925293599689). A later 17-pair validation request made the plugin-refresh preflight explicit because [the marketplace can leave the bundled metric catalog stale silently](https://razorpay.slack.com/archives/C0A98PQTJH4/p1786420608675429). On 1 September, [all 68 payments-infra KAM metrics switched to redesign tables](https://razorpay.slack.com/archives/C0A98PQTJH4/p1788259038806079), and the cutover note warned users to update the plugin or keep reading the old catalog; automated catalog syncs landed in [`claude-plugins` #1277](https://github.com/razorpay/claude-plugins/pull/1277) and [#1278](https://github.com/razorpay/claude-plugins/pull/1278). That makes refresh a routine post-cutover check, not only a shadow-validation step. A 13 August investigation exposed the other freshness trap: [a daily downstream load made roughly 22 metrics look current while their manually rebuilt source had not changed since 4 August](https://razorpay.slack.com/archives/C0A98PQTJH4/p1786612966405379). On 17 August, [a Mid Market pair showed the control itself can be wrong](https://razorpay.slack.com/archives/C0A98PQTJH4/p1797520898391909): legacy counted only successful attempts while redesign counted all intended attempts. On 27 August, [`self-serve-analytics` #2239](https://github.com/razorpay/self-serve-analytics/pull/2239) proved a one-day producer was feeding 30-day consumers: per-day row counts matched while a derived value silently changed. A same-day [`#ai-help` recovery](https://razorpay.slack.com/archives/C08C35GKJKD/p1787816521545829) restored the latest date but stamped 13 backfilled days into two partitions. The repository now guards the producer window with a custom test—the same assertion pattern documented in [dbt data tests](https://docs.getdbt.com/docs/build/data-tests). On 10 September, [a 58-metric Rize validation request](https://razorpay.slack.com/archives/C0A98PQTJH4/p1789015335403639) found one redesign reading roughly 1.2% low because both queries ended on “today” while legacy rebuilt at 12:15 and redesign at 08:00. On 15 September, merged [`self-serve-analytics` #2514](https://github.com/razorpay/self-serve-analytics/pull/2514) found a shared historical date where legacy was 10.2% short: a missing-date scan could not see it, a naive full-history scan produced 1,044 pre-floor false flags, and raw attempt counts produced false positives where the metric's distinct entity sets still matched. The method was verified against no-code and refunds before merge. Freshness belongs to the producing data, completeness belongs to every shared interval, coverage belongs to the full consumer window, and correctness belongs to the approved metric intent—not whichever query shipped first.

The meaning check became concrete on 16 September. An [`#analytics-self-serve` audit](https://razorpay.slack.com/archives/C0A98PQTJH4/p1789555865541679) found four metrics labelled as MCP usage without any MCP-attribution field, two differently named metrics backed by the same measure, and one “rate” returning 300% because numerator and denominator used different grains. A [parallel activation review](https://razorpay.slack.com/archives/C0A98PQTJH4/p1789550057790819) produced 37 versus 177 entities—roughly a 5× difference—from two plausible milestones. Those are contract failures before they are SQL failures. dbt's [semantic-model guidance](https://docs.getdbt.com/docs/build/semantic-models) similarly makes entities, dimensions, and measures explicit so downstream metrics share declared meaning instead of relying on labels alone.

Open [`self-serve-analytics` #2561](https://github.com/razorpay/self-serve-analytics/pull/2561) exposed the range trap on 16 September: a recent overlap window passed exactly, but the promoted unbounded `total_salary_transferred_monthly` path served ₹19,152 Cr against ₹36,904 Cr from legacy—49% missing for weeks—and six sibling metrics had the same exposure. A comparison window can prove computation only inside that window. Coverage must be proved against the query's requested range before any value match earns sign-off.

The empty-set trap surfaced on 18 September. In an [`#analytics-self-serve` validation run](https://razorpay.slack.com/archives/C0A98PQTJH4/p1789728760418379), two metrics returned zero rows on both legacy and redesign, so the comparison scored them as passes while exercising no evidence; the owner had to decide whether each definition should be corrected or retired. dbt's [unit-test guidance](https://docs.getdbt.com/docs/build/unit-tests) likewise recommends explicit input fixtures and expected outputs, including empty-input and edge-case tests. Equality is useful only after the validation slice proves it contains the evidence the metric contract expected.
